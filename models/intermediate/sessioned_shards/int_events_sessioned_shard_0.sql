{{
    config(
        ...
        post_hook=[
            "create index if not exists idx_int_events_sessioned_shard_0_customer_dt on {{ this }} (customer, dt)",
            "create index if not exists idx_int_events_sessioned_shard_0_customer_start on {{ this }} (customer, dt desc) where is_session_start = 1"
        ]
    )
}}
...
{#
    ...
    (prod.events_partitioned_p0) rather than a dbt ref() -- dbt has no
...
{# Force dbt to run int_events_partitioned (which loads prod.events_partitioned_p0) before this shard -- these shards read a raw partition table, not a ref(), so dbt has no other way to know the dependency. #}
...
events as (
    ...
    from prod.events_partitioned_p0 e
